...
mon_canton <- surfaces_ra1852 %>% 
  filter(canton == "LILLE") %>% ### Changer ici avec votre canton
...
```{r}
Controle_superf <- mon_canton %>%
  filter(name %in% c("cereales","racines_et_legumes_divers",
                     "cultures_diverses","prairies_artificielles","jacheres")) %>% 
  summarise(value = sum(value))

Controle_superf_aux <- mon_canton %>%
  filter(name %in% c("total_des_terres_labourables","prairies_naturelles",
                     "vignes","autres_cultures_arborescentes","paturages",
                     "autres_surfaces_ecoumene")) %>% 
  summarise(value = sum(value))

mon_canton_aux <- mon_canton %>% 
  pivot_wider() %>% 
  mutate(SOMME_TL = sum(cereales+racines_et_legumes_divers+
                     cultures_diverses+prairies_artificielles+jacheres,na.rm = T),
         EC = total_des_terres_labourables - SOMME_TL)
```
...
### Attention : le terminal inverse les coordonnées géographiques par rapport à GoogleEarth

ZZZ_1852_aux1 <- surfaces_ra1852 %>% 
  filter(is.na(departement) == T) %>% 
  mutate(code_new1 = code/10,
         code_new2 = as.character(code_new1),
         code_new3 = str_pad(code_new2,2,pad="0",side="left")) %>% 
  select(code_departement = code_new3,canton)
           
ZZZ_1852_aux2 <- surfaces_ra1852 %>% 
  filter(is.na(departement) == F)
  
ZZZ_1852_aux3 <- ZZZ_1852_aux2 %>%
  left_join(ZZZ_1852_aux1, by = c("departement" = "canton"))

###ZZZ_1852_aux5 <- ZZZ_1852_aux3 %>%
###  mutate(canton_aux = str_replace_all(canton," ","-")) %>%
###  select(canton = canton_aux)
...
surfaces_ra1852_coordonnees_centre <- ZZZ_1852_aux3 %>%
...
Temporaire <- surfaces_ra1852_coordonnees_centre %>% filter(centre == "NULL")
...
print(liste_coordonnees_non_trouvees %>% select(canton, departement, code_departement, centre))
...
Table_lat_long <- surfaces_ra1852_coordonnees_centre_corrigees %>% 
  mutate(longitude_aux=str_split_fixed(centre,",",2)[,1],
         latitude_aux=str_split_fixed(centre,",",2)[,2])

Table_lat_long <- Table_lat_long %>% 
  mutate(longitude = as.numeric(substr(longitude_aux,3,nchar(longitude_aux)))) %>% 
  mutate(latitude = as.numeric(substr(latitude_aux,1,nchar(latitude_aux)-1)))
```
...
leaflet(data = Table_lat_long) %>% 
  addTiles() %>%
  addMarkers(lng = ~longitude, 
             lat = ~latitude, 
             popup = ~canton, 
             label = ~(paste0("Canton : ",canton," - Surfaces en céréales : ",cereales," ha"))) %>% 
  setView(lng = 5.69, lat = 45.74, zoom = 12)
```
...
# geo.api.gouv.fr --> Les coordonnées GeoJSON sont exprimées en WGS-84 (EPSG 4326)

# pour travailler sur des aspects géographiques dont les voronoi il faut passer du dataframe au géo dataframe
surfaces_ra1852_geo <-
  st_as_sf(
    Table_lat_long %>% drop_na(c(longitude, latitude)),
...
st_union() %>%
# permet de passer une simple à un seul objet géométrique avec tous les points
st_voronoi() %>%
# calcul du voronoi
st_collection_extract()
# extraction en une liste avec les 363 figures géométriques
